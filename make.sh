...
function run()
{
  docker run -d --name="${NAME}" -p 1025:1025 -p 1082:8025 ${DOCKER_USER}/${NAME}
}
...
function push()
{ 
  docker login -u jenkins -p jenkins ${DOCKER_REGISTRY}
...
function usage()
{
  ...
  echo "   push : pour faire un push sur dok1.jvsonline.fr"
